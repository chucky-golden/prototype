...
async function movePage(url) {
    try {
        ...
        document.querySelector("#load").innerHTML = response.data;

        const tempDiv = document.createElement('div');
        tempDiv.innerHTML = response.data;
        const newTitle = tempDiv.querySelector('title') ? tempDiv.querySelector('title').innerText : 'Banter';
...
window.addEventListener("popstate", async function() {
    let res = await changePage( this.window.location.pathname);
    document.querySelector("#load").innerHTML = res.data
} );